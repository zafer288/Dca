...
import { X, Wallet, Shield, Trash2, RefreshCw } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSaveGlobal = async () => {
    setLoading(true);
    try {
      // Sadece hesapları güncelliyoruz, webhook url vs. local simülasyonda sabit.
      await api.updateConfig({ accounts });
      setLoading(false);
      alert("✅ Ayarlar başarıyla kaydedildi (Yerel Mod).");
      onClose();
    } catch (err: any) {
      setLoading(false); 
